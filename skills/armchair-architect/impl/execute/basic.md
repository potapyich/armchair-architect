...
Bare-bones autonomous task executor. No external dependencies, no role specialization.
Reads `implementation_plan.json`, implements tasks one by one, verifies each, marks done.

The recommended executor is `default.md`, which extends this one with role-aware
subagent prompts for parallel dispatch. Use `basic.md` directly only if you want
to disable specialization entirely.
...
**On pass:** Update `implementation_plan.json` — set `passes: true`, clear `in_progress`, then emit a `task_pass` event with the attempt count (1 if first try, 2 if recovered after a fix):

```bash
cat implementation_plan.json | jq '
  map(if .id == "<task_id>" then .passes = true | .in_progress = false else . end)
' > /tmp/plan_update.json && mv /tmp/plan_update.json implementation_plan.json
PYTHONPATH=${CLAUDE_SKILL_DIR}/lib python3 -m state emit-event task_pass id=<task_id> attempts=<1|2> category=<task_category>
```

Announce:
> Task <id> passed verification.

**On stuck (2 failed fix attempts):** Clear `in_progress`, do NOT mark as passed, then emit a `task_stuck` event. Go to Stuck Task handling below.

```bash
cat implementation_plan.json | jq '
  map(if .id == "<task_id>" then .in_progress = false else . end)
' > /tmp/plan_update.json && mv /tmp/plan_update.json implementation_plan.json
PYTHONPATH=${CLAUDE_SKILL_DIR}/lib python3 -m state emit-event task_stuck id=<task_id> category=<task_category>
```
...
```bash
PYTHONPATH=${CLAUDE_SKILL_DIR}/lib python3 -c "import state; s=state.load() or {}; print(s.get('impl',{}).get('review_every',0))"
```
...
4. On escalation (D):
   ...
   - Roll back to the chosen step (moves the chosen step and all steps after it from `completed` back to `pending`, sets `step` to the chosen value):

```bash
PYTHONPATH=${CLAUDE_SKILL_DIR}/lib python3 -m state rollback-to <chosen_step>
PYTHONPATH=${CLAUDE_SKILL_DIR}/lib python3 -m state emit-event escalation task=<id> to=<chosen_step>
```
   - Tell the user:
...
Claude has no reliable way to read its own context usage. Instead of guessing a
percentage, snapshot progress on a fixed cadence: after every **5 completed tasks**
(or whenever you start a new run after a handoff), write `progress.md` and remind
the user they can switch to a fresh session if responses begin feeling truncated.
...
> Snapshot after task <id>. Wrote `progress.md` with current execution state.
>
> If responses start feeling truncated, start a new session and run
> `/armchair-architect` — it will resume from task <next_id>.
